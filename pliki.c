...
int main(void){
   ...
   wsk=fopen("mojplik.txt", "w");
   x=fwrite((const void *)&s1, sizeof(struct student), 1, wsk);         //podajemy w kolejności: adres danych które mają być zapisane; rozmiar tablicy elementu; liczba el do zapisu; plik, na ktorym wykonywana jest operacja
   x=fwrite((const void *)&s2, sizeof(struct student), 1, wsk);
   x=fwrite((const void *)&s3, sizeof(struct student), 1, wsk);
   x=fwrite((const void *)&s4, sizeof(struct student), 1, wsk);
   x=fwrite((const void *)&s5, sizeof(struct student), 1, wsk);
   fclose(wsk);
